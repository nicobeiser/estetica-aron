...
const slides = document.querySelectorAll('.slider-item-masajes');
if (slides.length > 1) {
    // Inicializas Swiper y la lógica de paginación y navegación
    const swiper = new Swiper(".slider-container", {
        effect: "fade",
        speed: 1300,
        navigation: {
    ...
    // Resto de la lógica de eventos para los tabs...
    sliderTabs.forEach((tab, index) => {
        tab.addEventListener("click", () => {
    ...
    // Si se especifica startSlide en la URL
    if (!isNaN(startSlide)) {
    ...
    // Solo hay un slide: ocultamos controles que no hacen falta
    document.querySelector('.slider-controls').style.display = 'none';
    document.querySelector('.slider-navigations').style.display = 'none';
    // Actualizamos manualmente el estado inicial, si es necesario:
    updateIndicator(sliderTabs[0], 0);
    updateActiveTab(0);
}
